...
const TeamBar = ({ team, rtl, defeatTeamName }: TeamBarProps) => {
    ...
    return (
        <div 
            className={`flex flex-col w-full max-w-60 md:w-60 bg-[#0c1524]/85 backdrop-blur-md border border-white/10 rounded-xl p-3 shadow-[0_12px_40px_rgba(0,0,0,0.6)] transition-all duration-300 ${
                rtl ? "md:mr-16" : ""
            }`}
        >
                    ...
                    >
                        ...
                        <div className="min-w-0 flex flex-col justify-center">
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-wider block mt-0.5 ${
                                ...
                                {p.is_connected ? "Online" : "DC"}
                            </span>
                        </div>

                        {!p.is_connected && (
                            <RiSignalWifiErrorLine size={12} className="text-rose-400 animate-pulse shrink-0 ml-auto" />
                        )}
                    </div>
                ))}
            </div>
            ...
            <div className="w-full pt-2 border-t border-white/10">
                <div className={`flex items-baseline justify-between text-[9px] font-black uppercase tracking-[0.12em] text-gray-400 mb-1 ${
