...
from flask import abort, redirect, session, request, url_for
...
def _get_frontend_oauth_callback_url(status: str) -> str:
    return f"{settings.FRONTEND_OAUTH_CALLBACK_URL}?status={status}"
...
def oauth_callback(provider):
    ...
    provider_data = settings.OAUTH_PROVIDERS[provider]
